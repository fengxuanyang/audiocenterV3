...
public class AudioTokenFactory {
    public static AudioToken getAudioToken(FragmentActivity activity, PlayListItem item, MediaPlayerManager.MediaPlayerHandler mediaPlayer) {
        AudioToken audiotoken = null;
        switch (item.getAudioType()) {
            case Constants.AUDIO_TYPE_ALBUM:

                audiotoken = new AlbumToken(activity, mediaPlayer, item);
                break;
